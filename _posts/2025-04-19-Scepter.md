...
date: 2025-04-19
description: Scepter is an Hard-difficulty machine 
published: true
...
showmount -e scepter.htb Export list for scepter.htb: /helpdesk (everyone)

Ok nice, mount it :
```bash

➜  rune mount -t nfs -o vers=4,nolock 10.129.28.117:/helpdesk ./toto
...
```
> [!NOTE]
> The **passphrase** corresponds to the password cracked using **john**.  
> For the **Export Password**, simply press **Enter** (leave it empty).
>
> ```bash
> openssl pkcs12 -in cert.pfx -nocerts -out priv-key.pem
> ```
...
![Initial Nmap Scan](/assets/htb/s1.png)


And **a.carter** has **GenericAll** rights on **STAFF ACCESS CERTIFICATE@SCEPTER.HTB**
![Initial Nmap Scan](/assets/htb/s2.png)


Also, **d.baker** is in **staff group**
![Initial Nmap Scan](/assets/htb/s3.png)
...
```
![Initial Nmap Scan](/assets/htb/s4.png)
...
> [!NOTE]
> This step is expected to fail, and is used to demonstrate why the attack does not work.
...
```
    ...
    The SID is the recommended way and therefore counts as “Strong”. The DC uses this SID in strong implicit certificate mapping instead of the weak UPN and DNS mapping to ensure that the enrollee can only authenticate as themselves. The strong ones contain identifiers that you cannot reuse. For example, the mail attribute is not a unique protected attribute so it is possible to set the mail attribute to the value of another user.
...
```
![Initial Nmap Scan](/assets/htb/s5.png)
...
- The certificate template shows `CT_FLAG_NO_SECURITY_EXTENSION` in `msPKI-Enrollment-Flag` and `CT_FLAG_SUBJECT_ALT_REQUIRE_EMAIL` in `msPKI-Certificate-Name-Flag`
